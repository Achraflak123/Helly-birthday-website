...
const CountdownTimer = () => {
  ...
  const CountItem = ({ value, label }) => (
    <div className="flex flex-col items-center">
      <div className="glass-effect rounded-lg shadow-lg p-4 w-20 md:w-28 h-20 md:h-28 flex items-center justify-center border border-white/20 transform hover:scale-105 transition-transform duration-300">
        <span className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-pink-500 to-purple-500 bg-clip-text text-transparent">{value}</span>
      </div>
  ...
  return (
    <section id="countdown" className="py-20 relative overflow-hidden">
      ...
      <div className="absolute top-20 left-20 parallax-element" data-speed="-0.2">
        <Star className="h-6 w-6 text-pink-400 opacity-60" fill="#F9A8D4" />
      </div>
      <div className="absolute bottom-20 right-20 parallax-element" data-speed="0.1">
        <Sparkles className="h-8 w-8 text-purple-400 opacity-60" />
      </div>
      
      <div className="container mx-auto px-4 text-center relative z-10">
        <div className="mb-10 flex justify-center">
          <div className="bg-white/50 p-4 rounded-full shadow-md">
